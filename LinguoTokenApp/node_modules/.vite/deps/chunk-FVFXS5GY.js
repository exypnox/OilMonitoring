...
async function resolveAbiFromBytecode(contract) {
  const [{ resolveImplementation }, { extractIPFSUri }] = await Promise.all([
    import("./resolveImplementation-S4F5MUZL.js"),
    import("./extractIPFS-IYKEUHMB.js")
  ]);
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract } = await import("./read-contract-4WF22K7X.js");
...
async function resolveModularModuleAddresses(contract) {
  try {
    const { getInstalledModules } = await import("./getInstalledModules-2NAFOKMN.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-7P2VAX3R.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-U74FHUAZ.js"),
      import("./eth_estimateGas-FRM66ATH.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-YDA67EFQ.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-FVFXS5GY.js.map
