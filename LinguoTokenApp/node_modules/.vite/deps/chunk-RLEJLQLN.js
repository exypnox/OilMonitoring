...
async function populateEip712Transaction(options) {
  var _a;
  const { account, transaction } = options;
  let [data, to, value, gas, maxFeePerGas, maxPriorityFeePerGas, eip712] = await Promise.all([
    encode(transaction),
      ...
      params: [
        {
          ...
          eip712Meta: {
            ...eip712,
            gasPerPubdata: gasPerPubdata ? toHex2(gasPerPubdata) : toHex2(50000n),
...
function serializeTransactionEIP7122(transaction) {
  ...
  const serializedTransaction = [
    ...
    toHex2(chainId),
    from ?? "0x",
    gasPerPubdata ? toHex2(gasPerPubdata) : toHex2(gasPerPubdataDefault2),
    factoryDeps ?? [],
    customSignature ?? "0x",
    // EIP712 signature
    paymaster && paymasterInput ? [paymaster, paymasterInput] : []
...
//# sourceMappingURL=chunk-RLEJLQLN.js.map
