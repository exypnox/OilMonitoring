...
exports.IS_THIRDWEB_URL_CACHE = void 0;
...
const is_jwt_js_1 = require("./jwt/is-jwt.js");
...
function getClientFetch(client, ecosystem) {
    ...
    async function fetchWithHeaders(url, init) {
        ...
        if (isThirdwebUrl(url)) {
            ...
            const authToken = client.secretKey && (0, is_jwt_js_1.isJWT)(client.secretKey)
                ? client.secretKey
                : undefined;
            // secret key if secret key !== jwt
            const secretKey = client.secretKey && !(0, is_jwt_js_1.isJWT)(client.secretKey)
                ? client.secretKey
                : undefined;
...
exports.IS_THIRDWEB_URL_CACHE = new lru_js_1.LruMap(4096);
/**
 * @internal
 */
function isThirdwebUrl(url) {
    if (exports.IS_THIRDWEB_URL_CACHE.has(url)) {
        // biome-ignore lint/style/noNonNullAssertion: the `has` above ensures that this will always be set
        return exports.IS_THIRDWEB_URL_CACHE.get(url);
    }
    try {
        const { hostname } = new URL(url);
        try {
            // special case for localhost in development only
            if (process_js_1.IS_DEV) {
                if (hostname === "localhost") {
                    exports.IS_THIRDWEB_URL_CACHE.set(url, true);
                    return true;
                }
            }
        }
        ...
        exports.IS_THIRDWEB_URL_CACHE.set(url, is);
        return is;
    }
    catch {
        exports.IS_THIRDWEB_URL_CACHE.set(url, false);
        return false;
    }
}
